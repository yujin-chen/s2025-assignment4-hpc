        ...
        if profile_step == step:
            with profile(
                ...
                with record_function("grad_sync"):
                    # Non-overlapping all-reduce
                    for p in model.parameters():
                        if p.grad is not None:
                            dist.all_reduce(p.grad, op=dist.ReduceOp.AVG)
                optim.step()
            
            prof.export_chrome_trace(f"ddp_no_overlap_trace_{model_name}_rank{rank}.json")
        else:
            logits = model(x)
            loss = F.cross_entropy(logits.view(-1, VOCAB_SIZE), y.view(-1))
            loss.backward()
...
def run_all_configs():
    ...
    for model_name, cfg in MODEL_CONFIGS.items():
        ...
        print(f"\nModel: {model_name}")
        print(f"Avg step time without overlap: {step_ms:.3f} ms")
